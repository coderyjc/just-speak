...
5. 实时使用时选择麦克风并点击“开始录音”；再次点击可暂停，继续时会建立新的流式连接，点击停止后才会进入文本处理。
...
- 实时录音可在同一条记录中暂停和继续；暂停会关闭当前流式 API，继续时重新建立连接。默认使用空格开始/暂停、`S` 停止并完成，可在设置中改绑或停用，且仅在实时录音页生效。
...
- 云端配置可保存为场景并随时应用或删除；历史记录默认保留最新 50 条，可在设置中调整为 10～600 条。超出上限时会清理最旧记录及对应任务文件，主页累计统计继续保留。
...
遇到 `NO_INPUT_AUDIO_ERROR` 时，先确认音频中有人声，再检查设置中的语言是否与素材一致；不确定语种时选择“自动识别”。该错误会立即停止当前任务并保留待处理片段，避免重复提交相同音频。
