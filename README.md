...
JustSpeak 是一款面向 Windows 的开源桌面语音转文字客户端。它将麦克风录音和音视频文件转换为文字，并可通过大语言模型继续完成文本清晰与定向修复。
...
- **文本处理流水线**：停止录音后依次保留“实时录音 → 文本清晰 → 定向修复”三个阶段，可随时切换查看。
...
克隆仓库后双击 `start.bat`。首次运行会自动调用 `setup.ps1`，创建 `.venv` 并安装依赖。
...
| LLM Model ID | 可选；留空会跳过“文本清晰”和“定向修复” |
...
主页统计采用累加式台账，涵盖语音输入总字数、实时录音与文件转写时长、最长单条、峰值日期、活跃天数及最近七天使用情况。费用按音频时长 `0.00033 元/秒` 估算；仅在接口响应包含 Token 用量时展示 Token 统计。

### 实时录音

选择麦克风后开始录音。暂停会结束当前云端连接并保留本次记录，再次开始时创建新连接；点击停止后进入两次可选的 LLM 文本处理。三个阶段的文本都会保存到同一条历史记录。
