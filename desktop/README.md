...
应用以单例模式运行，同一时间只会保留一个 JustSpeak 进程；重复启动会唤醒已经运行的主界面。应用启动后会常驻 Windows 通知区域。关闭主界面会将其隐藏到托盘；左键单击托盘图标可重新打开主界面。右键菜单提供“打开主界面”“显示/关闭悬浮窗”和“关闭”，选择“关闭”会结束应用及正在运行的任务。

主界面左侧栏底部的“Mini”可切换到单行悬浮窗，主界面与 Mini 窗口始终只显示一个。Mini 默认在任意应用中按右 Alt 开始录音；按下时立即触发，长按只触发一次。录音开始后再次按同一快捷键会停止录音并继续文本清洗与定向修复，完成后结果自动复制到剪贴板。完成后第一次按快捷键会重置流程，第二次按快捷键开始下一轮。快捷键可在设置页的“快捷键”区域自行录入。Mini 窗口处于活动状态时，空格用于暂停或继续，Esc 用于终止并重置流程。主界面实时录音页也可按 Esc 终止并重置，随后按默认空格快捷键重新开始。
...
Mini 用“读取屏幕、正在录音、文本清洗、定向修复、完成”五个单行状态同步显示同一条实时流程。录音期间会根据麦克风输入音量显示透明炫彩波形；右侧主页按钮可退出 Mini 并直接返回主界面主页。悬浮胶囊可拖动，并保持在其他窗口上方。
