...
class HomePage(Page):
    def __init__(self) -> None:
        ...
        heatmap_footer.setSpacing(1)
        self.heatmap_period_group = QButtonGroup(self)
        self.heatmap_period_group.setExclusive(True)
        self.heatmap_period_buttons: dict[str, QPushButton] = {}
        for period, label in (("year", "年"), ("month", "月")):
            button = QPushButton(label)
            button.setObjectName("heatmapRangeButton")
            button.setCheckable(True)
            button.setFocusPolicy(Qt.FocusPolicy.NoFocus)
            button.setAccessibleName(f"显示最近一{label}的使用情况")
            button.setToolTip(f"显示最近一{label}的使用情况")
            button.clicked.connect(
                lambda checked=False, value=period: self._set_heatmap_period(value)
            )
            self.heatmap_period_group.addButton(button)
            self.heatmap_period_buttons[period] = button
            heatmap_footer.addWidget(button)
            if period == "year":
    ...
    def set_statistics(self, stats: dict[str, object]) -> None:
        ...
        self._heatmap_daily = daily if isinstance(daily, dict) else {}
        self._heatmap_as_of = str(stats.get("as_of_date") or "")
        self._render_heatmap_period(animate=False)
        valid = realtime_count + file_count
        self.set_page_status(f"已统计 {valid} 条有效文稿", "success" if valid else "idle")

    def _set_heatmap_period(self, period: str) -> None:
        if period not in {"year", "month"}:
            return
        changed = period != self._heatmap_period
        self._heatmap_period = period
        self.heatmap_period_buttons[period].setChecked(True)
        self._render_heatmap_period(animate=changed)

    def _render_heatmap_period(self, animate: bool = True) -> None:
        is_month = self._heatmap_period == "month"
        self.heatmap.set_range_days(30 if is_month else 365)
        self.heatmap.set_data(self._heatmap_daily, self._heatmap_as_of)
        as_of = self._heatmap_as_of
        self.heatmap_range.setText(
            f"{'30 天' if is_month else '12 个月'} · 至 {as_of[5:].replace('-', '.')}"
            if as_of
            else ("过去 30 天" if is_month else "过去 12 个月")
        )
        if animate:
            animate_reveal(self.heatmap, 165, 0.38)
...
class PipelineNav(QWidget):
    ...
    def select(self, stage: str) -> None:
        ...
        if button and button.isEnabled():
            changed = not button.isChecked()
            button.setChecked(True)
            if changed:
                animate_reveal(button, 135, 0.58)
...
class RealtimePage(Page):
    ...
    def __init__(self) -> None:
        ...
        self.device.setSizePolicy(
            QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Fixed
        )
        self.device.setToolTip("选择要使用的麦克风")
        self.screen = NoWheelComboBox()
        self.screen.setMinimumWidth(0)
        self.screen.setSizePolicy(
            QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Fixed
        )
        self.screen.setToolTip("选择信息增强要截取的显示器")
        self.screen.setVisible(False)
        self.input_selectors = QWidget()
        self.input_selectors.setObjectName("realtimeInputSelectors")
        self.input_selector_layout = QHBoxLayout(self.input_selectors)
        self.input_selector_layout.setContentsMargins(0, 0, 0, 0)
        self.input_selector_layout.setSpacing(0)
        self.input_selector_layout.addWidget(self.device, 1)
        self.input_selector_layout.addWidget(self.screen, 1)
        self.input_selectors.setMinimumWidth(180)
        control_row.addWidget(self.input_selectors, 1)
        self.refresh = _icon_button(
            QStyle.StandardPixmap.SP_BrowserReload, "刷新麦克风与屏幕列表"
        )
        ...
        self.transcript.setObjectName("readonlyTranscript")
        self.transcript.setAccessibleName("只读转写阶段内容")
        ...
        self._information_enhancement_enabled = False
    ...
    def set_information_enhancement_enabled(self, enabled: bool) -> None:
        self._information_enhancement_enabled = bool(enabled)
        self.screen.setVisible(enabled)
        self.input_selector_layout.setSpacing(7 if enabled else 0)
        if (
            self._recording_state == "idle"
            and not any(self._stage_texts.values())
            and not self.can_copy_and_reset()
        ):
            self.reset_pipeline(context_enabled=enabled, started=False)

    def set_recording_state(self, state: str) -> None:
        if state not in {"idle", "preparing", "recording", "paused", "stopping"}:
            return
        ...
        self.screen.setEnabled(not active and self.screen.count() > 0)
        ...
        if recording:
            ...
            self.set_page_status("正在读取屏幕上下文…", "live")
        else:
    ...
    def _refresh_recording_tooltips(self) -> None:
        if self._recording_state == "recording":
            ...
            action = "继续录音"
        elif self._recording_state == "preparing":
            action = "正在准备录音"
        elif self.can_copy_and_reset():
            action = "复制定向修复结果并清屏"
        else:
            action = "开始录音"
        toggle_suffix = (
    ...
    def reset_pipeline(
        self, context_enabled: bool = False, started: bool = False
    ) -> None:
        """Reset the pipeline into either a ready state or an active first stage."""
        self._stage_texts = {stage: "" for stage, _ in PIPELINE_STAGES}
        self._stage_states = {stage: "pending" for stage, _ in PIPELINE_STAGES}
        self._selected_stage = "context" if context_enabled else "asr"
        self._final_stage = "asr"
        for stage, _ in PIPELINE_STAGES:
            self.pipeline.set_stage(stage, "pending", False)
        if context_enabled:
            context_state = "running" if started else "pending"
            self._stage_states["context"] = context_state
            self.pipeline.set_stage("context", context_state, True)
            self.pipeline.select("context")
        else:
            self._stage_states["context"] = "skipped"
            self.pipeline.set_stage("context", "skipped", False)
            asr_state = "running" if started else "pending"
            self._stage_states["asr"] = asr_state
            self.pipeline.set_stage("asr", asr_state, True)
            self.pipeline.select("asr")
        self.transcript.clear()
        self._set_stage_placeholder(self._selected_stage)
        self.transcript.setReadOnly(True)
        self._refresh_recording_tooltips()
    ...
    def set_stage_state(
        ...
        if state == "running" and activate_running:
    ...
    def can_copy_and_reset(self) -> bool:
        return (
            not self._recording_running
            and self._stage_states.get("polish") == "completed"
            and bool(self._stage_texts.get("polish", "").strip())
        )

    def final_text(self) -> str:
        return self._stage_texts.get(self._final_stage, "").strip()

    def next_round_status(self) -> str:
        action = self._toggle_shortcut_hint or "录音按钮"
        return f"已复制并清屏 · 再按{action}开始下一轮"

    def reset_for_next_round(self) -> None:
        self.duration.setText("00:00.0")
        self.reset_pipeline(
            context_enabled=self._information_enhancement_enabled,
            started=False,
        )
        self.set_recording_state("idle")
    ...
    def _select_stage(self, stage: str) -> None:
        ...
        self._set_stage_placeholder(stage)
        self.transcript.setPlainText(self._stage_texts.get(stage, ""))
        animate_reveal(self.transcript, 155, 0.35)
        self._update_editor_access()

    def _update_editor_access(self) -> None:
        self.transcript.setReadOnly(True)

    def _set_stage_placeholder(self, stage: str) -> None:
        context_placeholder = (
            "点击开始录音或按空格读取屏幕上下文…"
            if self._stage_states.get("context") == "pending"
            else "正在读取所选屏幕的文字上下文…"
        )
        placeholders = {
            "context": context_placeholder,
            "asr": "开始说话后，识别文字会在这里逐句出现…",
            "clarity": "录音完成后将在这里显示文本清洗结果…",
            "polish": "文本清洗完成后将在这里显示定向修复结果…",
        }
        self.transcript.setPlaceholderText(placeholders.get(stage, ""))
...
class HistoryPage(Page):
    ...
    def __init__(self) -> None:
        ...
        left = QVBoxLayout()
        left.setSpacing(0)
        self.sessions = AnimatedListWidget()
        ...
        self.detail_panel.setObjectName("historyDetailPanel")
        right = QVBoxLayout(self.detail_panel)
    ...
    def set_pipeline(
        ...
        animate_reveal(self.detail_panel, 175, 0.28)
    ...
    def _select_stage(self, stage: str, preserve_current: bool = True) -> None:
        ...
        if preserve_current:
            animate_reveal(self.text, 155, 0.35)
...
class SettingsPage(Page):
    ...
    def __init__(
        ...
        cloud_layout.addWidget(
            _label("同一密钥串联语音识别、屏幕 OCR 与两次文本处理", "cardCaption")
        )
        ...
        enhancement = Card("settingsCard")
        enhancement_layout = QVBoxLayout(enhancement)
        enhancement_layout.setContentsMargins(20, 18, 20, 18)
        enhancement_layout.setSpacing(11)
        enhancement_header = QHBoxLayout()
        enhancement_header.addWidget(_label("信息增强", "cardTitle"))
        enhancement_header.addStretch(1)
        self.information_enhancement = QCheckBox("启用")
        self.information_enhancement.setChecked(
            config.information_enhancement_enabled
        )
        enhancement_header.addWidget(self.information_enhancement)
        enhancement_layout.addLayout(enhancement_header)
        enhancement_layout.addWidget(
            _label(
                "开始实时录音时截取选定显示器，将 OCR 文字追加为语音识别上下文",
                "cardCaption",
            )
        )
        self.ocr_model = QLineEdit(config.ocr_model)
        self.ocr_model.setPlaceholderText("例如 qwen3.5-ocr")
        self.ocr_base_url = QLineEdit(config.ocr_base_url)
        self.ocr_base_url.setPlaceholderText(
            "留空按地域与 Workspace 生成 /compatible-mode/v1"
        )
        self.ocr_base_url.setToolTip(
            "可填写专属服务的 OpenAI 兼容 Base URL；也可包含 /chat/completions"
        )
        ocr_endpoint_row = QHBoxLayout()
        ocr_endpoint_row.setContentsMargins(0, 0, 0, 0)
        ocr_endpoint_row.setSpacing(12)
        ocr_endpoint_row.addWidget(_settings_field("OCR Model ID", self.ocr_model), 1)
        ocr_endpoint_row.addWidget(_settings_field("OCR Base URL", self.ocr_base_url), 2)
        enhancement_layout.addLayout(ocr_endpoint_row)
        self.ocr_context_max_chars = NoWheelSpinBox()
        self.ocr_context_max_chars.setRange(
            MIN_OCR_CONTEXT_LENGTH, MAX_OCR_CONTEXT_LENGTH
        )
        self.ocr_context_max_chars.setSuffix(" 字")
        self.ocr_context_max_chars.setValue(
            max(
                MIN_OCR_CONTEXT_LENGTH,
                min(MAX_OCR_CONTEXT_LENGTH, config.ocr_context_max_chars),
            )
        )
        self.ocr_context_max_chars.setToolTip(
            "OCR 文字的取用上限；与定向转写 Prompt 合计仍不超过 400 字符"
        )
        self.ocr_prompt = QPlainTextEdit(config.ocr_prompt or DEFAULT_OCR_PROMPT)
        self.ocr_prompt.setObjectName("promptEditor")
        self.ocr_prompt.setFixedHeight(82)
        self.ocr_prompt.setPlaceholderText(DEFAULT_OCR_PROMPT)
        ocr_prompt_row = QHBoxLayout()
        ocr_prompt_row.setContentsMargins(0, 0, 0, 0)
        ocr_prompt_row.setSpacing(12)
        ocr_prompt_row.addWidget(_settings_field("OCR 提取指令", self.ocr_prompt), 2)
        ocr_prompt_row.addWidget(
            _settings_field("上下文上限", self.ocr_context_max_chars), 1
        )
        enhancement_layout.addLayout(ocr_prompt_row)
        enhancement_layout.addWidget(
            _label(
                "截图只在内存中处理并发送到配置的 OCR 接口，不会保存为本地图片",
                "cardCaption",
            )
        )
        self._enhancement_controls = (
            ...
            self.ocr_base_url,
            self.ocr_prompt,
            self.ocr_context_max_chars,
        )
        self.information_enhancement.toggled.connect(
            self._sync_enhancement_controls
        )
        self._sync_enhancement_controls(
            self.information_enhancement.isChecked()
        )
        columns.addWidget(enhancement)
        ...
        for signal in (
            ...
            self.information_enhancement.toggled,
            self.ocr_model.textChanged,
            self.ocr_base_url.textChanged,
            self.ocr_prompt.textChanged,
            self.ocr_context_max_chars.valueChanged,
    ...
    def values(self) -> AppConfig:
        return AppConfig(
            ...
            information_enhancement_enabled=self.information_enhancement.isChecked(),
            ocr_model=self.ocr_model.text().strip() or "qwen3.5-ocr",
            ocr_base_url=self.ocr_base_url.text().strip(),
            ocr_prompt=self.ocr_prompt.toPlainText().strip() or DEFAULT_OCR_PROMPT,
            ocr_context_max_chars=self.ocr_context_max_chars.value(),
    ...
    def apply_config(self, config: AppConfig) -> None:
        ...
        try:
            ...
            self.ocr_model.setText(config.ocr_model)
            self.ocr_base_url.setText(config.ocr_base_url)
            self.ocr_prompt.setPlainText(config.ocr_prompt or DEFAULT_OCR_PROMPT)
            self.ocr_context_max_chars.setValue(
                max(
                    MIN_OCR_CONTEXT_LENGTH,
                    min(MAX_OCR_CONTEXT_LENGTH, config.ocr_context_max_chars),
    ...
    def _sync_enhancement_controls(self, enabled: bool) -> None:
        for control in self._enhancement_controls:
