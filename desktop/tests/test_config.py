...
def test_endpoint_generation() -> None:
    ...
    assert AppConfig(workspace_id="ws3").ocr_endpoint() == (
        "https://ws3.cn-beijing.maas.aliyuncs.com/compatible-mode/v1"
    )
    assert AppConfig(
        ocr_base_url="https://ocr.example/compatible-mode/v1"
    ).ocr_endpoint() == "https://ocr.example/compatible-mode/v1"
...
def test_config_round_trip_contains_no_secret(tmp_path) -> None:
    ...
    assert loaded.ocr_context_max_chars == 400
...
def test_scenarios_save_apply_replace_and_delete(tmp_path) -> None:
    ...
    assert applied.information_enhancement_enabled is False
