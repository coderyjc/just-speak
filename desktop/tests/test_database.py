...
def test_session_list_includes_final_character_count(tmp_path) -> None:
    database = Database(tmp_path / "history.sqlite3")
    session = database.create_session(
        "realtime", "实时录音", tmp_path / "task", {}
    )
    database.save_edited_text(session, "一二 三\n四", track_usage=True)

    row = database.list_sessions()[0]

    assert row["char_count"] == 4
    database.close()
